...
import {
  ...
  SmartToy as SmartToyIcon, ExpandMore as ExpandMoreIcon
...
const googleMapsApiKey = import.meta.env.VITE_GOOGLE_MAPS_API_KEY || 'AIzaSyDEyAiggQthZ3AibDKDY9UNBoypYYamKBo';
...
export default function MapaCasos() {
  ...
  const [showIAAnalysis, setShowIAAnalysis] = useState(false); // Alterado para false
  ...
  const additionalControls = useMemo(() => (
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', ml: 2 }}>
      <Tooltip title="Recarregar dados">
        <IconButton color="inherit" onClick={carregarMaisDados} disabled={loading}>
          {loading ? <CircularProgress size={24} color="inherit" /> : <RefreshIcon />}
        </IconButton>
      </Tooltip>
      
      <Tooltip title="Filtrar por data">
        <IconButton color="inherit" onClick={() => setShowDateFilter(true)}>
          <DateRangeIcon />
        </IconButton>
      </Tooltip>
      ...
      >
        <ToggleButton value="casos" sx={{ px: 2 }}>
          <PeopleIcon fontSize="small" sx={{ mr: 1 }} />
          Casos
        </ToggleButton>
        <ToggleButton value="agrupado" sx={{ px: 2 }}>
      ...
      <Tooltip title={showHeatmap ? "Desativar mapa de calor" : "Ativar mapa de calor"}>
        <IconButton 
          ...
          sx={{ 
            bgcolor: showHeatmap ? 'rgba(0, 229, 255, 0.1)' : 'transparent',
      ...
      <Tooltip title="Insights e Análise Inteligente (IA)">
        <IconButton 
          ...
          sx={{ 
            bgcolor: showIAAnalysis ? 'rgba(0, 229, 255, 0.1)' : 'transparent',
            '&:hover': { bgcolor: 'rgba(255,255,255,0.1)' }
          }}
        >
          <SmartToyIcon />
      ...
      <Tooltip title="Exportar para CSV">
        <IconButton color="inherit" onClick={exportToCSV}>
          <GetAppIcon />
        </IconButton>
      </Tooltip>

      <Tooltip title="Exportar para PDF (com insights IA)">
        <IconButton color="inherit" onClick={exportToPDF} disabled={loading}>
          <GetAppIcon />
        </IconButton>
      </Tooltip>
    </Box>
  ), [loading, viewMode, showHeatmap, mapType, showIAAnalysis]);
    ...
    <ThemeProvider theme={darkTheme}>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', bgcolor: 'background.default' }}>
          ...
          {/* Painel de Estatísticas (Agora aparece abaixo do menu fixo) */}
          ...
          {/* Painel de Insights e Análise IA (Agora é um Accordion) */}
            ...
            <Box sx={{ flex: 1, position: 'relative', bgcolor: '#0a0a0a' }}>
              ...
              >
                <Typography variant="subtitle2" fontWeight="bold" sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  {viewMode === 'agrupado' ? <LocationCityIcon fontSize="small" /> : <PeopleIcon fontSize="small" />}
                  {viewMode === 'agrupado' ? 'Bairros' : 'Casos'} Ativos
                </Typography>
                ...
                {dateFilter && (
                  <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
                    Filtrado: {format(dateFilter, 'dd/MM/yyyy')}
                  </Typography>
                )}
              </Card>
            </Box>
          ...
          {/* Controles do Mapa */}
          <Box sx={{ position: 'fixed', top: 120, right: 16, display: 'flex', flexDirection: 'column', gap: 1, zIndex: 1000 }}>
